#!/usr/bin/env python3
"""
Claude Batch Builder for AITA MCQ Evaluation
Reads JSONL benchmark files and creates Claude Batch API format.
"""
...
"""
    ...
    prompt += f"\n\nAnswer with ONLY the letter ({answer_format}). No explanation needed.\n\nYou must choose one of the options, even if information appears incomplete. Guess if needed. The model is not allowed to respond with uncertainty or lack of information."
    
    return prompt


def build_batch_request(mcq: Dict[str, Any], custom_id: str, 
                       model_name: str = "claude-sonnet-4-20250514",
                       max_tokens: int = 100,
                       temperature: float = 0.1) -> Dict[str, Any]:
    """Build a single batch request for Claude API"""
...
def process_jsonl_file(file_path: Path, output_dir: Path, 
                      model_name: str, max_tokens: int, temperature: float) -> int:
    ...
    with open(file_path, 'r', encoding='utf-8') as f_in, \
        ...
        for line in f_in:
            ...
            # Build batch request
            request = build_batch_request(mcq, custom_id, model_name, max_tokens, temperature)
...
Example usage:
  python claude_batch_gen.py \\
    ...
    for file_path in jsonl_files:
        request_count = process_jsonl_file(
            file_path, output_dir, args.model, args.max_tokens, args.temperature
        )
